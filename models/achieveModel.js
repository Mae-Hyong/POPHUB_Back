...
const insert_achieveHub_query =
    "INSERT IGNORE INTO achieve_hub (user_name, achieve_id) VALUES (?, ?)";
...
const select_achieveHub_query =
    "SELECT * FROM achieve_hub WHERE user_name = ? AND achieve_id = ?";
const select_achieve_query = "SELECT * FROM achieve WHERE achieve_id = ?";
const count_mark_query =
    "SELECT COUNT(*) AS markCount FROM BookMark WHERE user_name = ?";
                ...
                (err, result) => {
                    ...
                    db.query(
                        ...
                        (err, achieveResult) => {
                            ...
                            if (achieveResult.length > 0) {
                                const achieveName = achieveResult[0].name;
                                const message = `${userName}님 ${achieveName} 업적을 달성했습니다!`;
